 ...
 */
public class MapReduceTDWFetcherHadoop2 extends MapReduceFetcher {
    ...
    private static final String TIMESTAMP_DIR_FORMAT = "%04d%02d%02d%02d";
    ...
    public MapReduceTDWFetcherHadoop2(FetcherConfigurationData fetcherConfData) throws IOException {
    ...
    protected String getHistoryDir(AnalyticJob job) {
        ...
        logger.info(String.format("job finish timestamp is %s", job.getFinishTime()));
    ...
    private DataFiles getHistoryFiles(AnalyticJob job) throws IOException {
        final String jobId = Utils.getJobIdFromApplicationId(job.getAppId());
        ...
        if (_fs.exists(new Path(jobHistoryDirPath))) {
            jobConfPath = jobHistoryDirPath + jobId+"_conf.xml";
            if (!_fs.exists(new Path(jobConfPath))) {
                throw new FileNotFoundException("Can't find config of " + jobId + " in "
                        + jobHistoryDirPath );
            }
            logger.info("Found job config in intermediate dir: " + jobConfPath);
            jobHistPath = jobHistoryDirPath + jobId+".jhist";
            if (!_fs.exists(new Path(jobConfPath))) {
                throw new FileNotFoundException("Can't find history file of " + jobId + " in "
                        + jobHistoryDirPath );
            }
        }
